...
class UWBParticleFilter(Node) :
    ...
    def __init__(self) :
        ...
        self.turtles_mocaps         = np.array([np.zeros(2) for _ in turtles])
        ...
        self.odoms_init             = np.array([0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001])
    ...
    def update_filter(self) :
        ...
        if all(self.turtles_odoms_flag) and  not self.pf_init_flag:
            # Create filter
            for t, to in enumerate(self.turtles_odoms):
                self.odoms_init[2*t] = to.pose.pose.position.x
                self.odoms_init[2*t + 1] = to.pose.pose.position.y
            print(self.odoms_init)
            # self.prior_fn = lambda n: self.odoms_init + np.random.normal(0,0.2,(n,8)) #np.random.uniform(-8,8,(n,8))+self.odoms_init
            
            priors = self.turtles_mocaps[1:] - np.array([self.turtles_mocaps[0][0], self.turtles_mocaps[0][1]])
            print(self.turtles_mocaps)
            # priors = self.turtles_mocaps - np.array([self.turtles_mocaps[0][0], self.turtles_mocaps[0][1]])
            priors = np.append(priors, np.array([-7,0]))
            print(priors)
            self.prior_fn = lambda n: priors.flatten() + np.random.normal(0,0.2,(n,8)) 
        ...
        if self.pf_init_flag:
            ...
            # self.particle_odom[6] = 0
            # self.particle_odom[7] = 0
            # print("---------------")
            sys.stdout.write("..")
            sys.stdout.flush()
            ...
            for i in range(8) :
                if self.pf.cov_state[i][0] > 0.3 or self.pf.cov_state[i][1] > 0.3 :
                    self.get_logger().warn("PF covariance too high for Turtle {} with covx={} and covy={}".format(i, self.pf.cov_state[0], self.pf.cov_state[1]))
            ...
            pf_relative_poses = [self.pf.mean_state[0], self.pf.mean_state[1], self.pf.mean_state[2], self.pf.mean_state[3], self.pf.mean_state[4], self.pf.mean_state[5], self.pf.mean_state[6], self.pf.mean_state[7]]
            ...
            print("UWB Ranges = {}".format(self.uwb_ranges))
            
            p = self.pf.mean_state
            print("Estimated mean state ranges = {}".format([
                    np.linalg.norm(p[0:2] - p[2:4]),
                    np.linalg.norm(p[0:2] - p[4:6]),
                    np.linalg.norm(p[0:2] - p[6:8]),
                    np.linalg.norm(p[2:4] - p[4:6]),
            ...
            p = np.array([
                self.turtles_mocaps[0][0],
                self.turtles_mocaps[0][1],
                self.turtles_mocaps[1][0],
                self.turtles_mocaps[1][1],
                self.turtles_mocaps[2][0],
                self.turtles_mocaps[2][1],
                self.turtles_mocaps[3][0],
                self.turtles_mocaps[3][1]
            ])
            print("MOCAP ranges = {}".format([
                    np.linalg.norm(p[0:2] - p[2:4]),
                    np.linalg.norm(p[0:2] - p[4:6]),
                    np.linalg.norm(p[0:2] - p[6:8]),
                    np.linalg.norm(p[2:4] - p[4:6]),
