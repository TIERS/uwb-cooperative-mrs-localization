...
real_poses_topics = ["/real_turtle01_pose", "/real_turtle03_pose", "/real_turtle04_pose"]
tri_poses_topics  = ["/tri_turtle01_pose", "/tri_turtle03_pose", "/tri_turtle04_pose"]
pf_poses_topics   = ["/pf_turtle01_pose", "/pf_turtle03_pose", "/pf_turtle04_pose"]

real_poses, uv_real_poses, tri_poses, pf_u_poses, pf_uv_poses   = [], [], [], [], []
align_tri_poses, align_pf_u_poses, align_pf_uv_poses   = [], [], []

with Rosbag2Reader("/home/xianjia/Workspace/temp/uwb_ranging_refine_with_spatial_detection/result_bags/20221011/cali_4robots_data_02/result_bag_0_0") as reader:
...
for inx in range(len(tri_poses)):
    # sync
...
ape_metrics_tri   = [metrics.APE(pose_relation) for _ in range(3)]
ape_metrics_pf_u  = [metrics.APE(pose_relation) for _ in range(3)]
ape_metrics_pf_uv = [metrics.APE(pose_relation) for _ in range(3)]
...
for i in range(3):
    ...
    traj_by_label = {
        ...
        "groundtruth_robot{}".format(i): align_tri_poses[i][0]
    }
