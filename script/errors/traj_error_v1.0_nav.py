...
pf_poses_topics   = "/pf_turtle05_pose"
...
ape_metrics_pf_u   = metrics.APE(pose_relation)

if use_aligned_trajectories:
    data = (traj_ref_u, traj_est_u)
ape_metrics_pf_u.process_data(data)
ape_stat = ape_metrics_pf_u.get_statistic(metrics.StatisticsType.rmse)
# print(ape_stat)
ape_stats = ape_metrics_pf_u.get_all_statistics()
# pprint.pprint(ape_stats)
np.savetxt("./results/ape/nav_u.csv", ape_metrics_pf_u.error)
...
traj_by_label = {
    ...
    "groundtruth_robot": data[0]
}
plot.trajectories(fig, traj_by_label, plot.PlotMode.xy)

FILENAME = "nav_traj" 
plt.savefig('./results/figs/img/nav_{}.png'.format(FILENAME), bbox_inches='tight')   
...
# with Rosbag2Reader(f"/home/xianjia/Workspace/temp/uwb_ranging_refine_with_spatial_detection/pf_ut") as reader:
#     for inx, topic in enumerate(real_poses_topics):
...
# # with Rosbag2Reader(f"/home/xianjia/Workspace/temp/uwb_ranging_refine_with_spatial_detection/pf_bak/results/result_bags/{bag_name}/result_bag_1_0") as reader:
# with Rosbag2Reader(f"/home/xianjia/Workspace/temp/uwb_ranging_refine_with_spatial_detection/pf_utv") as reader:
#     for inx, topic in enumerate(real_poses_topics):
...
# ape_metrics_pf_u   = [metrics.APE(pose_relation) for _ in range(1)]
...
#     # pprint.pprint(ape_stats)
#     np.savetxt("./results/ape/ape_tri_t{}.csv".format(inx), ape_metrics_tri[inx].error)
...
#     # pprint.pprint(ape_stats)
#     np.savetxt("./results/ape/ape_triu_t{}.csv".format(inx), ape_metrics_triu[inx].error)

# print(len(align_pf_u_poses))
...
#     # pprint.pprint(ape_stats)
#     np.savetxt("./results/ape/nav_u_t{}.csv".format(inx), ape_metrics_pf_u[inx].error)
...
#     # pprint.pprint(ape_stats)
#     np.savetxt("./results/ape/ape_ut_t{}.csv".format(inx), ape_metrics_pf_ut[inx].error)
...
#     # pprint.pprint(ape_stats)
#     np.savetxt("./results/ape/ape_utv_t{}.csv".format(inx), ape_metrics_pf_utv[inx].error)
