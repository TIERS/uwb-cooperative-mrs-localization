...
ape    = [pos_list[0], pos_list[4], pos_list[8], pos_list[12], pos_list[1], pos_list[5], pos_list[9], pos_list[13], pos_list[2], pos_list[6], pos_list[10], pos_list[14],  pos_list[3], pos_list[7], pos_list[11], pos_list[15]]
robots = [
          [0]*len(pos_list[0]), [0]*len(pos_list[4]),[0]*len(pos_list[8]),  [0]*len(pos_list[12]), 
          [1]*len(pos_list[1]), [1]*len(pos_list[5]),[1]*len(pos_list[9]),  [1]*len(pos_list[13]), 
          [2]*len(pos_list[2]), [2]*len(pos_list[6]),[2]*len(pos_list[10]), [2]*len(pos_list[14]), 
          [3]*len(pos_list[3]), [3]*len(pos_list[7]),[3]*len(pos_list[11]), [3]*len(pos_list[15]), ]
          
approaches = [
              [0]*len(pos_list[0]), [1]*len(pos_list[4]),[2]*len(pos_list[8]), [3]*len(pos_list[12]), 
              [0]*len(pos_list[1]), [1]*len(pos_list[5]),[2]*len(pos_list[9]), [3]*len(pos_list[13]), 
              [0]*len(pos_list[2]), [1]*len(pos_list[6]),[2]*len(pos_list[10]),[3]*len(pos_list[14]), 
              [0]*len(pos_list[3]), [1]*len(pos_list[7]),[2]*len(pos_list[11]),[3]*len(pos_list[15]), 
              ]       
...
dx = "robots"; dy = "ape_value"; dhue = "approaches"; pal = "Set2"; sigma = .6; ort = "v";cut=0.0
# f, ax = plt.subplots(figsize=(8,5))
f, ax = plt.subplots()

ax=pt.RainCloud(x = dx, y = dy, hue = dhue, data = df, palette = pal, orient=ort, bw = sigma, cut=cut, width_viol = .6,
