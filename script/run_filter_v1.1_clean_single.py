...
with mlp.Manager() as manager:
    ...
    for bag_file in bag_files:
        ...
        for i, fus in enumerate(fusion): 
            ...
            for r in tqdm.tqdm(range(3)):
                print("//////////////////////")
                p0 = sp.Popen(["ros2", "bag", "play", "recorded_data/{}".format(bag_file)], stdout=sp.PIPE, stderr=sp.STDOUT)
                time.sleep(0.1)
                p1 = sp.Popen(["python", "pfilter_ros2_uwb_position_single_range.py", "--fuse_group", "{}".format(i), "--robot_id", "{}".format(r)], stdout=sp.PIPE, stderr=sp.STDOUT)

                # p2 = sp.Popen(["python", "triangulation_ros2_uwb_position_v3.0.py", "--round", "{}".format(r)], stdout=sp.PIPE, stderr=sp.STDOUT)

                if os.path.exists("./result_bags/{}/single/result_bag_{}_{}".format(bag_file, i, r)):
                    shutil.rmtree("./result_bags/{}/single/result_bag_{}_{}".format(bag_file, i, r)) 
                # time.sleep(100)
                # time.sleep(150)
                print(f"robot: {robots[r]}")
                p3 = sp.Popen(["ros2", "bag", "record", "/real_turtle0{}_pose".format(robots[r]), "/pf_turtle0{}_pose".format(robots[r]), "-o", "./result_bags/{}/single/result_bag_{}_{}".format(bag_file, i, r)], stdout=sp.PIPE, stderr=sp.STDOUT)
                # p4 = sp.Popen([""])
                # print(f"/////// {p2.pid}")
                # tri_que = mlp.Queue()
                # pf_que  = mlp.Queue()
                ...
                np.savetxt("./results/single/pfilter/computation/mem_pf_{}_{}.csv".format(fus, r), np.array(pf_mems))
