...
class UWBParticleFilter(Node) :
    ...
    def calc_hypothesis(self, x) :
        ...
        if args.fuse_group == 1 and len(self.vis_meas_list) > 0:
            tmp = []
            for sp in range(len(self.sp_temp)):
                for _ in self.sp_temp[sp]:
...
def main(args=None):
    ...
    vis_timer = filter.create_timer(1/10.0, filter.update_vision_measurements)
    try:
        try:
            while rclpy.ok() :
                rclpy.spin(filter)             
