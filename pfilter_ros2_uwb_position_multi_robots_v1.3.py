...
def parse_args():
    ...
    parser.add_argument('--fuse_group', type=int, default=2, help='0: only UWB in PF, 1: with vision replace new measurement, 2: uwb and vision together')
...
class UWBParticleFilter(Node) :
    ...
    def __init__(self) :
        ...
        # Create filter
        self.prior_fn = lambda n: np.random.uniform(-8,2,(n,8))
        ...
        self.particle_odom          = np.array([0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01])
    ...
    def update_filter(self) :
        ...
        if args.fuse_group == 2 or args.fuse_group == 1:
            for i, p in enumerate(spatial_pair):
                ...
                # print("objects 0: {}, objects 1: {}".format(self.spatial_objects[p[0]].size, self.spatial_objects[p[1]].size))
                    ...
                    for obj0 in self.spatial_objects[p[0]]:
                        for obj1 in self.spatial_objects[p[1]]:
                            ...
                            if math.fabs(vis_meas -  self.uwb_ranges[spatial_uwb[p]]) < 0.5:
                                ...
                                # print(spatial_dict[p])
                                # print(f"dict size: {len(spatial_dict[p])}")
                                vis_meas_list.append(vis_meas)
                                # vis_meas_list.append(vis_meas)
        # print(f"vis mea: {vis_meas_list}")      
        ...
        for i in range(1, len(turtles)):
            self.particle_odom[2*(i-i)] = self.turtles_odoms[i].pose.pose.position.x - self.last_turtles_odoms[i].pose.pose.position.x - \
...
def main(args=None):
    ...
    # Start calculating relative positions
    filter_timer = filter.create_timer(1/20.0, filter.update_filter)
